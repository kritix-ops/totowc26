...
import { Newspaper, ArrowUpRight, Trophy, Sparkles } from "lucide-react";
...
export default async function DashboardPage({
  ...
  const [upcoming, lastFinal, rankInfo, trend, board] = await Promise.all([
    getUpcomingFixtures(user.id, 6),
    getLatestFinalForUser(user.id),
    getMyRankSummary(user.id),
    getPointsTrend(user.id),
  ...
  return (
    <section className="px-4 md:px-16 py-6 md:py-12 flex flex-col gap-8 md:gap-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
        <div className="lg:col-span-7 flex flex-col gap-12">
          <section className="flex flex-col gap-6">
            ...
            {upcoming.length === 0 ? (
              <EmptyMatches isHebrew={locale === "he"} />
            ) : (
        ...
        <div className="lg:col-span-5 flex flex-col gap-12">
            ...
            <Card className="p-5 flex items-center gap-4 min-h-[64px] hover:bg-surface-container transition-colors">
              ...
              <div className="flex flex-col min-w-0 flex-1">
                <span className="font-bold text-base text-on-surface">
                  {locale === "he" ? "הימורי על הטורניר" : "Tournament specials"}
                </span>
          ...
          <section className="flex flex-col gap-6">
            <div className="flex justify-between items-end">
              <SectionHeading>{locale === "he" ? "טבלת המובילים" : "Standings"}</SectionHeading>
              <Link
                href={localePath(locale, "leaderboard")}
                className="font-[family-name:var(--font-label)] text-[12px] font-bold tracking-[0.05em] text-primary hover:underline inline-flex items-center gap-1"
              >
                {locale === "he" ? "טבלה מלאה" : "Full table"}
                <ArrowUpRight className="h-3.5 w-3.5" strokeWidth={2} />
              </Link>
            </div>
...
> {
  ...
  // Include one above me and one below me for context, plus the separator.
  const result: Array<LeaderboardEntry | { separator: true; key: string }> = [...top3];
  const context = board.slice(Math.max(3, meIdx - 1), meIdx + 2);
  // If there's a real gap between top3 and the context window, insert ellipsis.
  if (context[0].rank > 4) {
