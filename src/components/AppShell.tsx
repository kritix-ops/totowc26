...
import { LayoutDashboard, ListChecks, Trophy, User, LogIn } from "lucide-react";
...
export async function AppShell({
  ...
  return (
    <>
        ...
        <div className="flex items-center gap-2 md:gap-4 shrink-0">
          {signedIn ? (
            <span className="hidden md:inline-block font-[family-name:var(--font-label)] text-[12px] font-bold tracking-[0.05em] text-on-surface-variant">
              {dict.common.rankShort} <bdi>#12</bdi>
            </span>
          ) : (
            <Link
      ...
      {signedIn && (
        <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-surface-container rounded-t-xl border-t border-outline-variant shadow-[0_-4px_12px_rgba(28,20,15,0.05)] grid grid-cols-4 items-stretch min-h-[64px] pb-[env(safe-area-inset-bottom)]">
